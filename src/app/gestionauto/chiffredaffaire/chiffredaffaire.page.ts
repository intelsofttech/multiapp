...
@Component({
  selector: 'app-chiffredaffaire',
  templateUrl: './chiffredaffaire.page.html',
  styleUrls: ['./chiffredaffaire.page.scss'],
})
export class ChiffredaffairePage implements OnInit {
 ...
 constructor(public alertCtrl: AlertController,
  ...
  onCalculrecette() {
	const adminamount = this.categForm.get('adminamount')?.value;
	const amount = this.categForm.get('amount')?.value;
	this.partneramount=Number(adminamount)-Number(amount);	
 ...
 onGetpartner(id:string){
  ...
  onAdd(): any{
	  ...
	  if (!this.categForm.valid) {
	   ...
	   const host = this.categForm.get('host')?.value;
	   const hostcode = this.categForm.get('hostcode')?.value;
	   const total = this.categForm.get('total')?.value;
	   const discount = this.categForm.get('discount')?.value;
	   const amounttaxe = this.categForm.get('amounttaxe')?.value;
	   const amountouttaxe = this.categForm.get('amountouttaxe')?.value;
	   const amount = this.categForm.get('amount')?.value;
	   const partneramount = this.categForm.get('partneramount')?.value;
	   const adminamount = this.categForm.get('adminamount')?.value;
 ...
 onGetcar(id:string){
	   ...
	   let postData = {
		...
		this.httpClient.get(this.REST_API_SERVER+"/car/"+id, {headers: header})
			.subscribe(data => {
				console.log(data);
				this.adminamount = data['recette'];
				this.host = data['owner'];
				this.hostcode = data['ownerid'];
				this.onGetconducteur(data['tenantid']);
			}, error => {
				console.log(error);
		});
 }
 ...
 onGetconducteur(id:string){
	   ...
	   let postData = {
		...
		this.httpClient.get(this.REST_API_SERVER+"/users/"+id, {headers: header})
			.subscribe(data => {
				this.partner = data['firstName']+" "+data['lastName'];
				this.partnercode = data['id'];
				this.phone1 = data['mobile'];
				this.email = data['emailaddress'];
				this.address = data['address'];
				this.city = data['city'];
					...
					.subscribe(data => {
						console.log(data);
							...
							this.results.forEach((line, index) => {
								...
								this.payamountTotal = Number(this.payamountTotal) + Number(line.adminamount);
								this.stayamountTotal = Number(this.stayamountTotal) + Number(line.partneramount);
							   if(!line.title) {
	   ...
	   let postData = {
		...
		this.httpClient.get(this.REST_API_SERVER+"/bill/"+id, {headers: header})
			.subscribe(data => {
				...
				this.partneramount = data['partneramount']; 
				this.adminamount = data['adminamount'];
