...
@Component({
  selector: 'app-newcotisation',
  templateUrl: './newcotisation.page.html',
  styleUrls: ['./newcotisation.page.scss'],
})
export class NewcotisationPage implements OnInit {
  ...
  ngOnInit() {
	...
	this.identitycard=  this.params.get('param1');
      ...
      this.isSend = false;
	   ...
	   var header = {
		...
		'Access-Control-Allow-Headers': 'Authorization, Origin, Content-Type, X-CSRF-Token',
			...
			.subscribe(data => {
					console.log(data);
					this.closemodal(this.identitycard,this.amount)
			}, error => {
					this.isSend = false;	
					console.log(error);
			});
