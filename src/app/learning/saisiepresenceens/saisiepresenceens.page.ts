...
@Component({
  selector: 'app-saisiepresenceens',
  templateUrl: './saisiepresenceens.page.html',
  styleUrls: ['./saisiepresenceens.page.scss'],
})
export class SaisiepresenceensPage implements OnInit {
  ...
  ngOnInit() {
   ...
   onHoraire(){
	   ...
	   const date = this.pageForm.get('date')?.value;
		...
		this.httpClient.get(this.REST_API_SERVER+"/configs/compte/ensid/date/"+this.typeemlpoi+"/"+this.param1+"/horaire/"+this.param2+"/"+date, {headers: header})
			.subscribe(data => {
				console.log(data);
				this.horaires = data['horaires'];
			}, error => {
