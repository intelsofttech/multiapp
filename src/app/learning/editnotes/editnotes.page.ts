...
listmode = false;
 ...
 onEditnote(){
	   ...
	   //this.httpClient.get(this.REST_API_SERVER+"/users/group3/student/"+this.param1, {headers: header})
		...
		console.log(this.REST_API_SERVER+"/users/group3/editnote/student/"+this.param2+"/"+this.param1+"/"+periode+"/"+matiere+"/"+this.year)
		 this.httpClient.get(this.REST_API_SERVER+"/users/group3/editnote/student/"+this.param2+"/"+this.param1+"/"+periode+"/"+matiere+"/"+this.year, {headers: header})
				.subscribe(data => {
							this.list = data;
							this.alllist = data;
							console.log(data);
 ...
 onUpdateNote(noteid:any, idel:string, event:any){
	console.log(noteid);
	console.log(event);
	...
	if(this.id=="0"){
		...
		let postData = {
			...
			this.httpClient.put(this.REST_API_SERVER+"/grade/"+this.id, postData, {headers: header})
			.subscribe(data => {
					console.log(data);
				}, error => {	
					console.log(error);
 ...
 onListperiodes(filiere:string){
  ...
  onListnote(){
		...
		this.httpClient.get(this.REST_API_SERVER+"/gradeconfig/level/order/"+this.param2, {headers: header})
			.subscribe(data => {
				console.log(data);
				this.notes = data;
			}, error => {
