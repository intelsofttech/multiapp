...
@Component({
  selector: 'app-typefrais',
  templateUrl: './typefrais.page.html',
  styleUrls: ['./typefrais.page.scss'],
})
export class TypefraisPage implements OnInit {
 ...
 onList(compte:string){
		...
		this.httpClient.get(this.REST_API_SERVER+"/configs/compte/"+this.param1+"/"+compte, {headers: header})
			.subscribe(data => {
				console.log(data);
				this.results = data;
				this.alllist = data;
 ...
 onParents(){
	   ...
	   let postData = {
		...
		this.httpClient.get(this.REST_API_SERVER+"/configs/"+this.param2, {headers: header})
			.subscribe(data => {
				console.log(data);
				this.parents = data;
				var filiere_html: any =document.getElementById("filiere")  as HTMLElement;
				this.compte= filiere_html.value;
				this.onList(this.compte);
			}, error => {
				console.log(error);
		});
