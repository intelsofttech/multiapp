...
@Component({
  ...
  templateUrl: './chambre.page.html',
  styleUrls: ['./chambre.page.scss'],
})
export class ChambrePage implements OnInit {
 ...
 onListTypechambre(){
  ...
  onAdd(): any{
	  ...
	  if (!this.categForm.valid) {
	   ...
	   const bedroomcateg = this.categForm.get('bedroomcateg')?.value;
	   this.isSend = true;
	   ...
	   if(this.id=="0"){
			let postData = {
				name: name,
				bedroomcateg: bedroomcateg,
				bedroomcategcode: bedroomcateg
		    }
			this.httpClient.post(this.REST_API_SERVER+"/bedroom", postData, {headers: header})
			.subscribe(data => {
					console.log(data);
					this.onList();
					this.addForm = false;
		   ...
		   this.httpClient.put(this.REST_API_SERVER+"/bedroom/"+this.id, postData, {headers: header})
				.subscribe(data => {
					console.log(data);
					this.onList();
					this.addForm = false;
 ...
 onDelete(id:string){
	   ...
	   let postData = {
		...
		this.httpClient.delete(this.REST_API_SERVER+"/bedroom/"+id, {headers: header})
			.subscribe(data => {
				console.log(data);
				this.onList();
			}, error => {
 ...
 onGet(id:string){
	   ...
	   let postData = {
		...
		this.httpClient.get(this.REST_API_SERVER+"/bedroom/"+id, {headers: header})
			.subscribe(data => {
				console.log(data);
				this.addForm = true;
				...
				this.id = data['id'];
				this.name = data['name'];
				this.bedroomcateg = data['bedroomcateg'];
				//this.onList();
			}, error => {
				console.log(error);
		});
