...
@Component({
  selector: 'app-validatebill',
  templateUrl: './validatebill.page.html',
  styleUrls: ['./validatebill.page.scss'],
})
	   ...
	   let postData = {
		...
		if(this.param2=="COUNTED"){
			if(this.param5=="0"){
				...
				this.httpClient.get(this.REST_API_SERVER+"/bills/type/counted/status/"+this.param1+"/"+this.counted+"/"+this.status, {headers: header})
					.subscribe(data => {
						console.log(data);
						this.results = data;
