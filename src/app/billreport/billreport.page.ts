...
@Component({
  ...
  templateUrl: './billreport.page.html',
  styleUrls: ['./billreport.page.scss'],
})
export class BillreportPage implements OnInit {
  ...
  baselink2 :any= "";
  baselink3 :any= "";
  libsublink1 :any= "Programme";
  libsublink2 :any= "Ilot";
  libsublink3 :any= "Lot";
  
  libbaselink1 :any= "Programme";
  libbaselink2 :any= "Ilot";
  libbaselink3 :any= "Lot";
  ...
  public projets :any = [];
  public programs :any = [];
  public programsbase :any = [];
  public apartments :any = [];
  public ilots :any = [];
  public lots :any = [];
  
  public apartmentsbase :any = [];
  public ilotsbase :any = [];
  public lotsbase :any = [];
 ...
 constructor(public alertCtrl: AlertController,
  ...
  ngOnInit() {
	...
	this.sublink3 = "0";
	if(this.param8=="1"){
	  this.libbaselink1 = "Programme";
	  this.libbaselink2 = "Ilot";
	  this.libbaselink3 = "Lot";
	  this.onProgramsbase("promotion");
	}else{
	  this.libbaselink1 = "Propriété";
	  this.libbaselink2 = "Bien immobilier";
	  this.libbaselink3 = "Type";
	  this.onProgramsbase("gestion");
	}
 ...
 async onChangebillmode() {
	this.billmode = !this.billmode;
	this.simpleform = !this.simpleform;
	console.log(this.billmode);
 }
 async ProductModal() {
	   ...
	   let postData = {
		...
		this.httpClient.get(this.REST_API_SERVER+"/product/"+id, {headers: header})
			.subscribe(data => {
				...
				const buyingprice = data['buyingprice'];
				const sellingprice = data['sellingprice'];
				const margin = data['margin'];
				const category = data['category'];
				const stockmini = data['stockmini'];
 ...
 onAddLine(){
  ...
  onChangelink() {
	const link = this.categForm.get('link')?.value;
	console.log(link)
	this.sublink1 = "0";
	this.sublink2 = "0";
	this.sublink3 = "0";
	if(link=="promotion"){
	  this.libsublink1 = "Programme";
	  this.libsublink2 = "Ilot";
	  this.libsublink3 = "Lot";
	}else{
	  this.libsublink1 = "Propriété";
	  this.libsublink2 = "Bien immobilier";
	  this.libsublink3 = "Type";
	
	}	
	this.onPrograms(link);
  }
  ...
  onChangesublink1() {
	...
	this.sublink3 = "0";
	const link = this.categForm.get('link')?.value;
	const sublink1 = this.categForm.get('sublink1')?.value;
	if(link=="promotion"){
	   this.apartments=[];
	   this.onListilot(sublink1);
	}else{
	   this.ilots=[];
	   this.onListapartement(sublink1);
	}
  }
  ...
  onChangesublink2() {
	...
	const link = this.categForm.get('link')?.value;
	const sublink1 = this.categForm.get('sublink1')?.value;
	const sublink2 = this.categForm.get('sublink2')?.value;
	if(link=="promotion"){
	   this.lots=[];
	   this.onListlot(sublink2);
	}else{
	   this.lots=[];
	}
  }
  ...
  onChangebasesublink1() {
	this.baselink2 = "0";
	this.baselink3 = "0";
	const baselink1 = this.selectForm.get('baselink1')?.value;
	if(this.param8=="1"){
	   this.apartmentsbase=[];
	   this.onListilotbase(baselink1);
	}else{
	   this.ilotsbase=[];
	   this.onListapartementbase(baselink1);
	}
	console.log("-"+this.baselink1+"-"+this.baselink2+"-"+this.baselink3+"-")
	this.onEditbill(this.baselink1, this.baselink2, this.baselink3);
  }
  
  onChangebasesublink2() {
	this.baselink3 = "0";
	const baselink1 = this.selectForm.get('baselink1')?.value;
	const baselink2 = this.selectForm.get('baselink2')?.value;
	if(this.param8=="1"){
	   this.lotsbase=[];
	   this.onListlotbase(baselink2);
	}else{
	   this.lotsbase=[];
	}
	console.log("-"+this.baselink1+"-"+this.baselink2+"-"+this.baselink3+"-")
  ...
  onChangebasesublink3() {
	const baselink1 = this.selectForm.get('baselink1')?.value;
	const baselink2 = this.selectForm.get('baselink2')?.value;
	const baselink3 = this.selectForm.get('baselink3')?.value;
	if(this.param8=="1"){
	   //this.onListlotbase(baselink2);
	}else{
...
onEditbill(baselink1:string, baselink2:string, baselink3:string){
    var header = {
			'Content-Type': 'application/json',
			'enctype': 'application/json',
			'Accept': '*',
   ...
   this._Url="";
   if(baselink3!="0"){
		this._Url=this.REST_API_SERVER+"/bills/type/categ/sublink3/"+this.param1+"/"+this.param2+"/"+baselink1+"/"+baselink3;
   }else{
		if(baselink2!="0"){
			this._Url=this.REST_API_SERVER+"/bills/type/categ/sublink2/"+this.param1+"/"+this.param2+"/"+baselink1+"/"+baselink2;
		}else{
		   if(baselink1!="0"){
				this._Url=this.REST_API_SERVER+"/bills/type/categ/sublink1/"+this.param1+"/"+this.param2+"/"+baselink1;
			}
		}
   }
   console.log(this._Url);
    this.httpClient.get(this._Url, {headers: header})
	.subscribe(data => {
			console.log(data);
			this.results = data;
			this.amountBill=0;
...
onProgramsbase(select:string){
	   ...
	   let postData = {
		...
		this.httpClient.get(this.REST_API_SERVER+"/property/category/"+select, {headers: header})
			.subscribe(data => {
				console.log(data);
				this.programsbase = data;
			}, error => {
				console.log(error);
		});
 } 
  
onPrograms(select:string){
		var header = {
			'Content-Type': 'application/json',
			'enctype': 'application/json',
			'Accept': '*',
 ...
 onListlotbase(ilot:any){
	   ...
	   let postData = {
		...
		this.httpClient.get(this.REST_API_SERVER+"/subdivision/parent/"+ilot, {headers: header})
		.subscribe(data => {
					console.log(data);
					this.lotsbase = data;
			}, error => {
 ...
 onListapartement(propriete:any){
		var header = {
			'Content-Type': 'application/json',
			'enctype': 'application/json',
			'Accept': '*',
	   ...
	   let postData = {
		...
		this.httpClient.get(this.REST_API_SERVER+"/apartment/property/"+propriete, {headers: header})
			.subscribe(data => {
				console.log(data);
 ...
 onListapartementbase(propriete:any){
		var header = {
			'Content-Type': 'application/json',
			'enctype': 'application/json',
			'Accept': '*',
  ...
  public actionSheetButtons = [
   ...
   async setResult(ev) {
	...
	if(this.result['data']['action']=="locataire"){
		this.selid = 'locataire';
		this.seltitle = 'Locataires';
		const modal = await this.modalCtrl.create({
			  component: PartnerPage,
			  componentProps: {
				'param1': this.selid,
	...
	if(this.result['data']['action']=="souscripteur"){
		this.selid = 'souscripteur';
		this.seltitle = 'Souscripteurs';
		const modal = await this.modalCtrl.create({
			  component: PartnerPage,
			  componentProps: {
				'param1': this.selid,
 ...
 onGetpartner(id:string){
  ...
  onAdd(): any{
	  ...
	  if (!this.categForm.valid) {
	   ...
	   const sublink1 = this.categForm.get('sublink1')?.value;
	   const sublink2 = this.categForm.get('sublink2')?.value;
	   const sublink3 = this.categForm.get('sublink3')?.value;
	   this.isSend = true;
	   ...
	   let postData = {
		...
		this.httpClient.get(this.REST_API_SERVER+"/bill/"+id, {headers: header})
			.subscribe(data => {
			    ...
			    this.link = data['link'];
			    this.sublink1 = data['sublink1'];
			    this.sublink2 = data['sublink2'];
			    this.sublink3 = data['sublink3'];
				//this.onList();
			}, error => {
				console.log(error);
		});
