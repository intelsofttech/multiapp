...
export class CommissionPage implements OnInit {
 ...
 constructor(public alertCtrl: AlertController,
  ...
  onCalculamount() {
	...
	this.amount=Number(this.total)-Number(this.discount);	
...
onGetDescash(id:string){
	   ...
	   let postData = {
		...
		this.httpClient.get(this.REST_API_SERVER+"/cashdesk/found/"+id, {headers: header})
			.subscribe(data => {
				...
				if(this.cashdesk!=""){
					this.showdescashbtn = true;
				}else{
					this.showdescashbtn = false;
				}
 ...
 onProperties(){
	   ...
	   let postData = {
		...
		this.httpClient.get(this.REST_API_SERVER+"/property/agency/"+this.agency, {headers: header})
			.subscribe(data => {
				...
				if(this._date1=="" || this._date2==""){	
					...
					.subscribe(data => {
						console.log(data);
							...
							this.results = data;
							this.amountBill = 0;
							this.comamountTotal = 0;
							this.chargeamountTotal = 0;
							this.results.forEach((line, index) => {
								 if(this.proprieteid=='0'){
									this.amountBill = Number(this.amountBill) + Number(line.amount);
									this.comamountTotal = Number(this.comamountTotal) + Number(line.comamount);
									this.chargeamountTotal = Number(this.chargeamountTotal) + Number(line.amounttaxe);
								 }else{
									 if(line.propertyparentid==this.proprieteid){
										this.amountBill = Number(this.amountBill) + Number(line.amount);
										this.comamountTotal = Number(this.comamountTotal) + Number(line.comamount);
										this.chargeamountTotal = Number(this.chargeamountTotal) + Number(line.amounttaxe);
									 }
 ...
 onList(){
	   ...
	   let postData = {
		...
		if(this.param2=="COUNTED"){
			...
			//console.log(this.REST_API_SERVER+"/bills/type/categ/"+this.param1+"/"+this.param2);
			this.httpClient.get(this.REST_API_SERVER+"/bills/type/categ/"+this.param1+"/"+this.param2, {headers: header})
				.subscribe(data => {
					console.log(data);
					this.results = data;
 ...
 onListperiod(){
	   ...
	   this.mois = this.pageForm.get('mois')?.value;
	   this.annee = this.pageForm.get('annee')?.value;
	   let postData = {
			
		}
		
		if(this.mois=="0"){
			this.httpClient.get(this.REST_API_SERVER+"/bills/type/counted/year/"+this.param1+"/COUNTED/"+this.annee, {headers: header})
				.subscribe(data => {
					console.log(data);
					this.results = data;
			...
			this.httpClient.get(this.REST_API_SERVER+"/bills/type/counted/period/"+this.param1+"/COUNTED/"+this.mois+"/"+this.annee, {headers: header})
				.subscribe(data => {
					console.log(data);
					this.results = data;
 ...
 onSendSms(phone:string, msg:string){
  ...
  onGetsmsPartner(id:string, msg:string){
		var header = {
			'Content-Type': 'application/json',
			'enctype': 'application/json',
			'Accept': '*',
	   ...
	   let postData = {
		...
		this.httpClient.get(this.REST_API_SERVER+"/users/"+id, {headers: header})
			.subscribe(data => {
				...
				this.email = data['emailaddress'];
				//this.onGetpartner(this.id);
				this.onSendSms(data['mobile'], msg);
			}, error => {
				console.log(error);
		});
 }
