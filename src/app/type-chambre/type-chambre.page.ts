...
@Component({
  ...
  templateUrl: './type-chambre.page.html',
  styleUrls: ['./type-chambre.page.scss'],
})
export class TypeChambrePage implements OnInit {
 ...
 constructor(public alertCtrl: AlertController,
  ...
  onAdd(): any{
	  ...
	  if (!this.categForm.valid) {
	   ...
	   const priceNight = this.categForm.get('priceNight')?.value;
	   const minPrice = this.categForm.get('minPrice')?.value;
	   const priceHour = this.categForm.get('priceHour')?.value;
	   const detail = this.categForm.get('detail')?.value;
	   this.isSend = true;
	   ...
	   if(this.id=="0"){
			...
			this.httpClient.post(this.REST_API_SERVER+"/bedroomcateg", postData, {headers: header})
			.subscribe(data => {
					console.log(data);
					this.onList();
					this.addForm = false;
		   ...
		   this.httpClient.put(this.REST_API_SERVER+"/bedroomcateg/"+this.id, postData, {headers: header})
				.subscribe(data => {
					console.log(data);
					this.onList();
					this.addForm = false;
 ...
 onDelete(id:string){
	   ...
	   let postData = {
		...
		this.httpClient.delete(this.REST_API_SERVER+"/bedroomcateg/"+id, {headers: header})
			.subscribe(data => {
				console.log(data);
				this.onList();
			}, error => {
