   ...
   async setResult(ev) {
	...
	if(this.result['data']['action']=="client"){
		...
		const modal = await this.modalCtrl.create({
			...
			modal.onDidDismiss().then(data=>{
				console.log(data)
					if(this.partselection=='edit'){
						this.partnercodeselect= data['data'];
						this.partnerselect= data['role'];
						this.onRefresh();
					}else{
	...
	if(this.result['data']['action']=="proprietaire"){
		this.selid = 'proprietaire';
		this.seltitle = 'Propriétaire';
		const modal = await this.modalCtrl.create({
			  component: PartnerPage,
			  componentProps: {
				'param1': this.selid,
 ...
 onRefresh(){
	...
	this.results.forEach((line, index) => {
								...
								if(this.partnercodeselect==""){
									...
									if(line.partnercode==this.partnercodeselect){
										this.amountBill = Number(this.amountBill) + Number(line.amount);
										this.payamountTotal = Number(this.payamountTotal) + Number(line.payamount);
										this.stayamountTotal = Number(this.stayamountTotal) + Number(line.stayamount);
									}
