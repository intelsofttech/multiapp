...
export class FirstPage implements OnInit {
 ...
 //usereglise  = [{id: 1,label: 'Eglise ou Département'},{id: 2,label: 'Entreprise et Métier'},{id: 3,label: 'Emploi et Stage'},{id: 4,label: 'Education et Encadrement'},{id: 5,label: 'Conseils et Suivi'}];
  ...
  backcolor3 :any = "#f1f0f0";
  textcolor :any = "#303234";
  textcolor1 :any = "#303234";
  textcolor2 :any = "#303234";
  textcolor3 :any = "#303234";
  ...
  goBilan() {
	   ...
	   let postData = {
		...
		this.httpClient.get(this.REST_API_SERVER+"/apartment/apikey", {headers: header})
						.subscribe(data => {
							...
							this.appartements.forEach((line, index) => {
								if(line.status=="Libre"){
									this.applibre = Number(this.applibre) + 1;
								}
								if(line.status=="Occupé"){
									this.appoccupe = Number(this.appoccupe) + 1;
								}
								if(line.status=="Fermé"){
									this.appferme = Number(this.appferme) + 1;
								}
								if(line.status=="Travaux"){
									this.apptravaux = Number(this.apptravaux) + 1;
								}
   ...
   onGetParner(){
	   ...
	   let postData = {
		...
		this.httpClient.get(this.REST_API_SERVER+"/users/partner/dette/locataire", {headers: header})
			.subscribe(data => {
							console.log(data);
							this.partners = data;
							this.partners.forEach((line, index) => {
								this.appall = Number(this.appall) + 1;
								if(line.motiva=="0"  || line.motiva==""){
									this.appactif = Number(this.appactif) + 1;
								}else{
									this.appinactif = Number(this.appinactif) + 1;
								}
							});
		...
		this.httpClient.get(this.REST_API_SERVER+"/users/partner/dette/locataire", {headers: header})
						.subscribe(data => {
							console.log(data);
								...
								this.loyers.forEach((line, index) => {
									this.impayeTotal = Number(this.impayeTotal) + Number(line.accountAmount);
   ...
   onBillRecette(){
	this.results = [];
	   ...
	   let postData = {
		...
		this.httpClient.get(this.REST_API_SERVER+"/bills/type/counted/dates/1/true"+"/"+this._date1+"/"+this._date2, {headers: header})
						.subscribe(data => {
							console.log(data);
								...
								this.results.forEach((line, index) => {
									this.recetteTotal = Number(this.recetteTotal) + Number(line.amount);
									this.margeTotal = Number(this.recetteTotal) - Number(this.chargeTotal);
   ...
   onBillCharge(){
	this.results = [];
	   ...
	   let postData = {
		...
		this.httpClient.get(this.REST_API_SERVER+"/bills/type/counted/dates/2/true"+"/"+this._date1+"/"+this._date2, {headers: header})
						.subscribe(data => {
							console.log(data);
								...
								this.chargeTotal=0;
								this.results.forEach((line, index) => {
									this.chargeTotal = Number(this.chargeTotal) + Number(line.amount);
									this.margeTotal = Number(this.recetteTotal) - Number(this.chargeTotal);
								});
						}, error => {
							console.log(error);
